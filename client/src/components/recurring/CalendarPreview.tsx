...
export function CalendarPreview() {
  ...
  const recurringDates = useMemo(() => {
    ...
    try {
      return generateRecurringDates(pattern as any, 365);
    } catch (error) {
  ...
  const getCalendarGrid = () => {
    ...
    const grid: Array<{ date: Date; isCurrentMonth: boolean; isToday: boolean }> = [];
      ...
      <div className="bg-gray-50 rounded-xl p-6">
        ...
        <div className="mt-4 p-4 bg-blue-50 rounded-lg">
          ...
          <p className="text-sm text-gray-700">
            {pattern.type ? formatRecurrencePattern(pattern as any) : "Select a recurrence pattern"}
          </p>
          {pattern.startDate && (
